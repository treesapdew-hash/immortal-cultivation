...
- **Ads** (`ads.gd`, TEST_MODE fake 5 s ad): Heavenly Fortune (5/day, Events → Daily), Free Summon, Extra Beast Hunt, daily ad chests (3/6/9). ×2 offline still TODO (needs the offline popup script).
...
1. `supabase_setup_1.sql` profiles + saves
2. `_2` sects, members, requests, chat, role actions
3. `_3` sign-in, donations, levels, research, treasury
4. `_4` Sect Trial ladder
5. `_5` server-owned shop (`sect_shop_items`), trial rewards, chat rate limit
6. `_6` `delete_my_account()`
...
## Open / next
- ×2 offline rewards ad (need offline popup script)
- Sect: account-safe moderation (report/filter), red dots on Guild, member activity, weekly ranking
